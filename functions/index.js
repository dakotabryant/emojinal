...
// const WEBHOOK_URL = 'https://requestb.in/rygb64ry';

// Reads the content of the node that triggered the function and sends it to the registered Webhook
// URL.

// exports.addHook = functions.https.onRequest((req, res) => {
//   const text = req.query.text;
//   admin.database().ref('/hooks').push({hello: text})
//   .then(snapshot => res.redirect(303, snapshot.ref))
...
// exports.algoTest = functions.https.onRequest((req, res) => {
//   let deck = new Deck(d);
//   console.log(deck);
//   deck.answeredQuestionCorrectly();
//   console.log(deck);
//   deck.answeredQuestionCorrectly();
//   console.log(deck);

//   admin.database().ref('/decks').push({deck1: deck})
//   .then(snapshot => res.redirect(303, snapshot.ref))
//   .catch(err => console.error(err));
// });
// /*junk function trying to get around the unique id passed in, currently doesnt work */
// exports.deckGrab = functions.https.onRequest((req, res) => {
//   admin.database().ref('/decks').once('value').then(snapshot => {
//     let dbDeck = snapshot.val();
//     let phDeck = new Deck(d);
//     // dbDeck.answeredQuestionCorrectly();
//     console.log('1  ', dbDeck['-KoOo6gPdw0h1IjzK2a9'].deck1.deck);
//     return phDeck.fetchEnd('now', dbDeck['-KoOo6gPdw0h1IjzK2a9'].deck1);
//   }).then(phDeck => {
//     console.log('2  ', phDeck);
//     return admin.database().ref('/decks').child('hello').push({item: phDeck});
//   }).then((snapshot) => res.redirect(303, snapshot.ref))
//   // res.status('200').json(dbDeck))
//   // .catch(err => console.error(err));
//   // admin.database().ref('/decks').child('hello').push({deck2: phDeck})
//   // .then((snapshot) => res.redirect(303, snapshot.ref))
//   .catch(err => console.error(err));
